...
function TabPanel(props: TabPanelProps) {
  ...
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`cron-jobs-tabpanel-${index}`}
      aria-labelledby={`cron-jobs-tab-${index}`}
      {...other}
    >
...
function a11yProps(index: number) {
  return {
    id: `cron-jobs-tab-${index}`,
    'aria-controls': `cron-jobs-tabpanel-${index}`
  }
}
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={value} onChange={handleChange} aria-label="cron jobs page tabs">
      ...
      <TabPanel value={value} index={1}>
        ...
        >
          <TextField label="Job Name" id="job-name-input" defaultValue="" size="small" />
          <TextField label="Job Type" id="job-type-input" defaultValue="" size="small" />
        </Box>
      </TabPanel>
    </Box>
  )
